...
namespace LawgistTheme\Inc\Classes;
...
class Lawgist_Main
{
    ...
    public function pagination()
    {
        ...
        echo '<div class="col-12"><div class="lawgist-navigation">';
    ...
    public function lawgist_theContentWithComment()
    {
    ...
    public function lawgist_breadcrumb_bridge()
    {
        $lawgist = get_option('lawgist');

        if (isset($lawgist['breadcrumb_on'])) :
            if ($lawgist['breadcrumb_on'] == true) :
                $this->lawgist_get_the_breadcrumbs();
            endif;
        else :
            $this->lawgist_get_the_breadcrumbs();
        endif;
     ...
     * @return breadcrumb
     */
    public function lawgist_get_the_breadcrumbs()
    {
        ...
        $title = $this->generateBreadCrumbTitle();
            ...
            if(is_single()){
                $lawgist_blog_image = !empty($lawgist['blog_bredcrumb_single_img']['url']) ? $lawgist['blog_bredcrumb_single_img']['url'] : '';
            }else{
                $lawgist_blog_image = !empty($lawgist['blog_bredcrumb_img']['url']) ? $lawgist['blog_bredcrumb_img']['url'] : '';
            }
        ...
        $output = '<div class="blog-breadcrumb" style="background-image: url('. $lawgist_blog_image .')">
        ...
        $output .= $this->lawgist_breadcrumbs();
        $output .= '
                        </div>
                    </div>
                </div>
    ...
    public function generateBreadCrumbTitle()
    {
        ...
        if (is_home() || is_front_page()) {
            $title = isset($lawgist['bp_title']) ? esc_html($lawgist['bp_title']) : esc_html__('Our Blog', 'lawgist');
        } elseif ('case-study' == lawgist_get_archive_post_type()) {
            $title = isset($lawgist['cs_title']) ? esc_html($lawgist['cs_title']) : esc_html__('Case Study', 'lawgist');
        } elseif ('job' == lawgist_get_archive_post_type()) {
            $title = isset($lawgist['job_title']) ? esc_html($lawgist['job_title']) : esc_html__(' Current job openings', 'lawgist');
        } elseif (is_page()) {
            $title = get_the_title();
        } elseif (is_single()) {
            $title = get_the_title();
        }elseif (function_exists('is_shop') && is_shop()) {
            $title = isset($lawgist['shop_title']) ? esc_html($lawgist['shop_title']) : esc_html__('Our Products', 'lawgist');
        } elseif (is_archive()) {
            $title = get_the_archive_title();
        } elseif (is_search()) {
            $title = esc_html__('Search results for: ', 'lawgist') . get_search_query();
        }elseif(is_404(  )){
            $title = isset($lawgist['404_title']) ? esc_html($lawgist['shop_title']) : esc_html__('404', 'lawgist');
        }

        return $title;
    }

    public static function lawgist_breadcrumbs()
    {

        $lawgist = get_option('lawgist');

        $sepOpt = (isset($lawgist['breadcrumb_sep']) ? $lawgist['breadcrumb_sep'] : '<i class="fa fa-angle-right"></i>');

        /* === OPTIONS === */
        $text['home'] = esc_html__('Home', 'lawgist'); // text for the 'Home' link
        $text['shop'] = esc_html__('Shop', 'lawgist'); // text for the 'Shop' link
        $text['category'] = esc_html__('Archive by Category "%s"', 'lawgist'); // text for a category page
        $text['search'] = esc_html__('Search Results for "%s" Query', 'lawgist'); // text for a search results page
        $text['tag'] = esc_html__('Posts Tagged "%s"', 'lawgist'); // text for a tag page
        $text['author'] = esc_html__('Articles Posted by %s', 'lawgist'); // text for an author page
        $text['404'] = esc_html__('Error 404', 'lawgist'); // text for the 404 page
        $text['page'] = esc_html__('Page %s', 'lawgist'); // text 'Page N'
        $text['cpage'] = esc_html__('Comment Page %s', 'lawgist'); // text 'Comment Page N'
